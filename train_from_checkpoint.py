...
def train(
    ...
    *   ``Tuple[PolicyClass, List[AgentID], Mapping[str, Any]]``: All agents that have
    ...
    # Restore policy weights from checkpoint
    restored_policy = ray.rllib.policy.Policy.from_checkpoint(policy_checkpoint)
    restored_policy_weights = restored_policy.get_weights()

    # class RestoreWeightsCallback(DefaultCallbacks):
    #     def on_algorithm_init(self, *, algorithm: "Algorithm", **kwargs) -> None:
    #         algorithm.set_weights({"prosumer_policy": restored_policy_weights})

    config = {
        "callbacks": CustomCallback(metrics, restored_policy_weights),
...
class CustomCallback(DefaultCallbacks):
    """RLlib callback that logs Phantom metrics and restores weights from a checkpoint."""
    
    def __init__(self, metrics: Mapping[str, "Metric"], restored_policy_weights) -> None:
        super().__init__()
        self.metrics = metrics
        self.restored_policy_weights = restored_policy_weights

    def on_algorithm_init(self, *, algorithm: "Algorithm", **kwargs) -> None:
        algorithm.set_weights({"prosumer_policy": self.restored_policy_weights})
    ...
    def __call__(self) -> "CustomCallback":
        return self
