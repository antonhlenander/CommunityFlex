...
from phantom.agents import Agent
from phantom.env import PhantomEnv
from phantom.metrics import Metric, logging_helper
from phantom.policy import Policy
from phantom.types import AgentID
from phantom.utils import check_env_config, rich_progress, show_pythonhashseed_warning
...
def train(
    ...
    high_level_rllib_config: Optional[Mapping[str, Any]] = None,
    low_level_rllib_config: Optional[Mapping[str, Any]] = None,
    ...
    *   ``Tuple[PolicyClass, List[AgentID], Mapping[str, Any]]``: All agents that have
    ...
    high_level_rllib_config = high_level_rllib_config or {}
    low_level_rllib_config = low_level_rllib_config or {}
    metrics = metrics or {}
    ...
    # Defaults for the high level and low level agent config
    default_config = {
    ...
    if algorithm == "PPO":
        default_config["sgd_minibatch_size"] = max(int(config["train_batch_size"] / 10), 1)

    high_level_config = default_config.copy()
    low_level_config = default_config.copy()

    high_level_config.update(high_level_rllib_config)
    low_level_config.update(low_level_rllib_config)

    high_level_config["multiagent"] = {
        "policies": policy_specs,
        "policy_mapping_fn": policy_mapping_fn,
        "policies_to_train": ["high_level_policy"],
    }

    low_level_config["multiagent"] = {
        "policies": policy_specs,
        "policy_mapping_fn": policy_mapping_fn,
        "policies_to_train": ["low_level_policy"],
    }

    high_level_config["multiagent"].update(high_level_rllib_config.get("multiagent", {}))
    low_level_config["multiagent"].update(low_level_rllib_config.get("multiagent", {}))

    high_level_config_obj = (
        ray.tune.registry.get_trainable_cls(algorithm)
        .get_default_config()
        .from_dict(config)
    )

    low_level_config_obj = (
        ray.tune.registry.get_trainable_cls(algorithm)
        .get_default_config()
        .from_dict(config)
    )
    ...
    # becomes stable.
    high_level_config_obj.rl_module(_enable_rl_module_api=False)
    high_level_config_obj.training(_enable_learner_api=False)
    low_level_config_obj.rl_module(_enable_rl_module_api=False)
    low_level_config_obj.training(_enable_learner_api=False)

    high_level_algo = high_level_config_obj.build()
    low_level_algo = low_level_config_obj.build()

    with rich_progress("Training...") as progress:
        for i in progress.track(range(1, iterations + 1)):
            result_high_level = high_level_algo.train()
            result_low_level = low_level_algo.train()
            ...
            if i == 1:
                high_level_config_dict = {
                    ...
                    "env_config": env_config,
                    "rllib_config": high_level_rllib_config,
                    "metrics": metrics,
                }
                low_level_config_dict = {
                    "algorithm": algorithm,
                    "env_class": env_class,
                    "iterations": iterations,
                    "checkpoint_freq": checkpoint_freq,
                    ...
                    "policies_to_train": policies_to_train,
                    "env_config": env_config,
                    "rllib_config": low_level_rllib_config,
                    "metrics": metrics,
                }


                with open(Path(high_level_algo.logdir, "phantom-high-level-training-params.pkl"), "wb") as f:
                    cloudpickle.dump(config, f)
                with open(Path(low_level_algo.logdir, "phantom-low-level-training-params.pkl"), "wb") as f:
                    cloudpickle.dump(config, f)

            if checkpoint_freq and (i % checkpoint_freq == 0 or i == iterations):
                checkpoint_path_high_level = Path(high_level_algo.logdir, f"checkpoint_{str(i).zfill(6)}")
                checkpoint_path_low_level = Path(low_level_algo.logdir, f"checkpoint_{str(i).zfill(6)}")
                high_level_algo.save(checkpoint_path_high_level)
                low_level_algo.save(checkpoint_path_low_level)

            high_level_algo.set_weights(low_level_algo.get_weights(["low_level_policy"]))
            low_level_algo.set_weights(high_level_algo.get_weights(["high_level_policy"]))

    print(f"Logs & checkpoints saved to: {high_level_algo.logdir}")
    print(f"Logs & checkpoints saved to: {low_level_algo.logdir}")

    return high_level_algo, low_level_algo
